...
MODE=${1:?usage: benchmark_selected_mode_vs_original.sh MODE CUTOFF RESULT_NAME INSTANCE_FILE}
CUTOFF=${2:?usage: benchmark_selected_mode_vs_original.sh MODE CUTOFF RESULT_NAME INSTANCE_FILE}
RESULT_NAME=${3:?usage: benchmark_selected_mode_vs_original.sh MODE CUTOFF RESULT_NAME INSTANCE_FILE}
SOURCE_INSTANCE_FILE=${4:?usage: benchmark_selected_mode_vs_original.sh MODE CUTOFF RESULT_NAME INSTANCE_FILE}
PARALLEL_PAIRS=5
TABU=1
BLOCK_SIZE=3
SEED_ARG=8
...
run_one() {
    ...
    start=$(date +%s.%N)
    timeout --signal=TERM --kill-after=5s "$((CUTOFF + 30))s" "$@" > "$log" 2>&1
    exit_code=$?
    end=$(date +%s.%N)

...
INSTANCE_FILE="/home/b520-2/Desktop/szm/IQCQP-block/LS-IQCQP/scripts/phase_f_instances.txt"
    ...
    for inst in "${instances[@]}"; do
        ...
        for seed in "${SEEDS[@]}"; do
            for config in "${configs[@]}"; do
                ...
                if [ -s "$logfile" ] && grep -q "best obj" "$logfile"; then
                    continue
                fi
...
echo "Total tasks to run for Phase F: $num_cmds"
if [ "$num_cmds" -gt 0 ]; then
    xargs -P 8 -I {} sh -c "{}" < "$commands_file"
fi

rm -f "$commands_file"
echo "Phase F experiments completed."

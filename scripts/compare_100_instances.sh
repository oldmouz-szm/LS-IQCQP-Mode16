...
# Read instances from existing file
mapfile -t instances < "$RESULTS_DIR/selected_instances.txt"


echo "Starting tests on 100 instances..." > "$RESULTS_DIR/status.txt"

for inst in "${instances[@]}"; do
  file="$DATA_DIR/$inst"
  
  # Run New (Phase F: mode 6, block_size 3, seed 8)
  start=$(date +%s.%N)
  $NEW_EXEC 10 1 "$file" 1 6 3 8 > "$RESULTS_DIR/${inst}_new.log" 2>&1
  end=$(date +%s.%N)
  wall=$(echo "$end - $start" | bc -l)
  echo "wall_time_seconds = $wall" >> "$RESULTS_DIR/${inst}_new.log"
  
  # Run Old (Legacy codebase)
  # start=$(date +%s.%N)
...
python3 "$RESULTS_DIR/parse_results.py"

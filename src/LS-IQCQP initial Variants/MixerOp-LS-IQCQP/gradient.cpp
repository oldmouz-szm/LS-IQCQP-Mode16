...
void qp_solver::gradient_random_walk_unsat_bin(bool has_grad, int best_op_index, Float best_op_score) {
  ...
  bool grad_wins = false;
  if (has_grad) grad_wins = (best_op_score > score);

  if (grad_wins) {
    gradient_execute_op_index(best_op_index, [this](int v, Float dv) { execute_critical_move_mix(v, dv); });
...
#ifdef GRADIENT_DEBUG
  std::cout << "[GRAD] random_walk_mix_not_dis choose: has_grad=" << has_grad << ", grad_score=" << best_op_score
            << ", single_score=" << score << std::endl;
#endif
}
// Random walk for SAT (bin with equal), mixing classic operators and a gradient op candidate
void qp_solver::gradient_random_walk_sat_bin_with_equal(bool has_grad, int best_op_index, Float best_op_score) {
  // Rebuild a light set of SAT operations similar to random_walk_sat_bin_with_equal, then compare with gradient op
  ...
  for (int vpos : rand_obj_idx) {
    obj_var = &(_vars[vpos]);
    Float cur_value = _cur_assignment[vpos];
    ...
    for (int con_pos : obj_var->constraints) {
      ...
      a_coeff = &(unbound_con->var_coeff[vpos]);
      if (unbound_con->is_equal)
        insert_var_change_value_sat_bin_equal(vpos, a_coeff, 0, unbound_con, linear_coeff_value, false);
      else if (a_coeff->obj_quadratic_coeff != 0 || a_coeff->obj_linear_coeff.size() != 0 ||
               _unbounded_constraints.find(con_pos) != _unbounded_constraints.end())
        insert_var_change_value_sat_bin(vpos, a_coeff, 0, unbound_con, linear_coeff_value, false);
    }
  }
  ...
  bool grad_wins = false;
  if (has_grad) grad_wins = (best_op_score > max_score);

  if (grad_wins) {
    gradient_execute_op_index(best_op_index, [this](int v, Float dv) { execute_critical_move_mix(v, dv); });
  } else if (max_score != INT32_MIN) {
    if (max_score == score)
      execute_critical_move_mix(var_pos, change_value);
    else {
      Float cv1 = (_cur_assignment[var_pos_equal_1] == 0) ? 1 : -1;
      Float cv2 = (_cur_assignment[var_pos_equal_2] == 0) ? 1 : -1;
      execute_critical_move_mix(var_pos_equal_1, cv1);
      execute_critical_move_mix(var_pos_equal_2, cv2);
    }
  } else {
  ...
  std::cout << "[GRAD] sat random walk choose: has_grad=" << has_grad << ", grad_score=" << best_op_score
            << ", single_or_pair_best=" << max_score << std::endl;
#endif
}

void qp_solver::gradient_random_walk_sat_bin(bool has_grad, int best_op_index, Float best_op_score) {
  polynomial_constraint* unbound_con;
  all_coeff* a_coeff;
  int var_idx;
  Float delta;
  ...
  for (int var_pos : rand_obj_idx) {
    ...
    for (int con_pos : obj_var->constraints) {
      ...
      if (a_coeff->obj_quadratic_coeff != 0 || a_coeff->obj_linear_coeff.size() != 0 ||
          _unbounded_constraints.find(con_pos) != _unbounded_constraints.end()) {
...
void qp_solver::gradient_random_walk_no_cons(bool has_grad, int best_op_index, Float best_op_score) {
  ...
  if (grad_wins) {
    gradient_execute_op_index(best_op_index, [this](int v, Float dv) { execute_critical_move_no_cons(v, dv); });
  } else if (score != INT32_MIN) {
    execute_critical_move_no_cons(var_pos, change_value_2);
  } else {
...
void qp_solver::gradient_random_walk_balance(bool has_grad, int best_op_index, Float best_op_score) {
  polynomial_constraint* unbound_con;
  all_coeff* a_coeff;
  int var_idx;
  Float delta;
  ...
  for (int var_pos : rand_obj_idx) {
    if (!_vars[var_pos].is_bin) {
      ...
      if (obj_var->obj_quadratic_coeff == 0) {
        ...
        else if (linear_coeff_value < 0)
          best_value = INT32_MAX;
        else
          continue;
      } else {
        if (obj_var->obj_quadratic_coeff > 0)
  ...
  Float score_equal;
  Float max_score;
  select_best_operation_with_pair_mix(var_pos_equal_1, var_1_change_value, var_pos_equal_2, var_2_change_value,
                                      score_equal);
  max_score = std::max(score, score_equal);
  bool grad_wins = false;
  if (has_grad) grad_wins = (best_op_score > max_score);
...
#ifdef GRADIENT_DEBUG
  std::cout << "[GRAD] random_walk_balance choose: has_grad=" << has_grad << ", grad_score=" << best_op_score
            << ", single_score=" << score << ", pair_score=" << score_equal << ", chosen="
            << (grad_wins ? "grad" : (max_score == score ? "single" : (max_score == score_equal ? "pair" : "noop")))
            << std::endl;
#endif
}

void qp_solver::gradient_random_walk_unsat_mix_not_dis(bool has_grad, int best_op_index, Float best_op_score) {
  polynomial_constraint* unsat_con;
  all_coeff* a_coeff;
  int var_idx;
  Float delta;
...
void qp_solver::gradient_bin_new_unsat_step() {
  ...
  bool grad_wins = false;
  if (has_grad) grad_wins = (best_grad_score > score);

  if (grad_wins && best_grad_score > 0) {
    gradient_execute_op_index(best_idx, [this](int v, Float dv) { execute_critical_move_mix(v, dv); });
  } else if (score > 0 && var_pos != -1) {
    execute_critical_move_mix(var_pos, change_value);
  } else {
    update_weight();
    gradient_random_walk_unsat_bin(has_grad, best_idx, best_grad_score);
  }
#ifdef GRADIENT_DEBUG
...
void qp_solver::gradient_bin_new_sat_step() {
  int var_pos;
  Float change_value, score;
  // single variable move in SAT phase
  insert_operation_sat_bin_with_equal();
  select_best_operation_bin(var_pos, change_value, score);
  // pair operator selection
  int var_pos_equal_1 = -1, var_pos_equal_2 = -1;
  Float change_value_not_used = 0, score_equal = INT32_MIN;
  select_best_operation_bin_with_pair(var_pos_equal_1, var_pos_equal_2, change_value_not_used, score_equal);

  // build gradient SAT pool
  ...
  // Choose the best among: grad, single, pair
  enum class Choice { None, Grad, Single, Pair } choice = Choice::None;
  Float best_score = INT32_MIN;
  if (score > best_score) {
    best_score = score;
    choice = Choice::Single;
  }
  if (score_equal > best_score) {
    best_score = score_equal;
    choice = Choice::Pair;
  }
  if (has_grad && best_grad_score > best_score) {
    best_score = best_grad_score;
    choice = Choice::Grad;
  }

  if (best_score > 0) {
    if (choice == Choice::Grad) {
      gradient_execute_op_index(best_idx, [this](int v, Float dv) { execute_critical_move_mix(v, dv); });
    } else if (choice == Choice::Single && var_pos != -1) {
      execute_critical_move_mix(var_pos, change_value);
    } else if (choice == Choice::Pair && var_pos_equal_1 != -1 && var_pos_equal_2 != -1) {
      Float cv1 = (_cur_assignment[var_pos_equal_1] == 0) ? 1 : -1;
      Float cv2 = (_cur_assignment[var_pos_equal_2] == 0) ? 1 : -1;
      execute_critical_move_mix(var_pos_equal_1, cv1);
      execute_critical_move_mix(var_pos_equal_2, cv2);
    ...
    update_weight();
    gradient_random_walk_sat_bin_with_equal(has_grad, best_idx, best_grad_score);
  }
#ifdef GRADIENT_DEBUG
...
void qp_solver::gradient_bin_sat_step() {
  ...
  // classic single-variable move in SAT phase (bin)
  ...
  // choose between single op and gradient op
  bool use_grad = has_grad && (best_grad_score > score);
  Float best_score = use_grad ? best_grad_score : score;
  ...
  if (best_score > 0) {
    ...
    update_weight();
    gradient_random_walk_sat_bin(has_grad, best_idx, best_grad_score);
  }
#ifdef GRADIENT_DEBUG
...
void qp_solver::gradient_no_cons_step() {
  int var_pos = -1;
  Float change_value = 0, score = INT32_MIN;

  // Single-variable candidate in no-cons setting
  ...
  if (best_score > 0) {
    if (use_grad) {
      ...
      execute_critical_move_no_cons(var_pos, change_value);
    }
  } else {
    ...
    if (!fps_move()) {
      ...
      gradient_random_walk_no_cons(has_grad, best_idx, best_grad_score);
    }
  }
#ifdef GRADIENT_DEBUG
...
void qp_solver::gradient_mix_sat_step() {
  ...
  // Classic mix-balance: single-variable and paired operations
  insert_operation_balance();
  select_best_operation_mix(var_pos, change_value, score);
  int var_pos_equal_1 = -1, var_pos_equal_2 = -1;
  Float var_1_change_value = 0, var_2_change_value = 0, score_equal = INT32_MIN;
  select_best_operation_with_pair_mix(var_pos_equal_1, var_1_change_value, var_pos_equal_2, var_2_change_value,
                                      score_equal);
  ...
  // Choose the best among: single / pair / gradient
  enum class Choice { None, Grad, Single, Pair } choice = Choice::None;
  Float best_score = INT32_MIN;
  if (score > best_score) {
    best_score = score;
  ...
  if (best_score > 0) {
    if (choice == Choice::Grad) {
      ...
      // Align with local_search_mix_balance: in feasible phase use execute_critical_move_mix_more for single move
    ...
    update_weight();
    gradient_random_walk_balance(has_grad, best_idx, best_grad_score);
  }
#ifdef GRADIENT_DEBUG
...
void qp_solver::gradient_mix_unsat_step() {
  ...
  // Single-variable UNSAT candidates (mix_not_dis)
  ...
  if (grad_wins && best_grad_score > 0) {
    ...
    // Consistent with mix_balance UNSAT phase: use execute_critical_move_mix_more for single move
    execute_critical_move_mix_more(var_pos, change_value);
  } else if (!compensate_move()) {
    update_weight();
    // UNSAT random walk supports mixed gradient candidates
    gradient_random_walk_unsat_mix_not_dis(has_grad, best_idx, best_grad_score);
  }
#ifdef GRADIENT_DEBUG

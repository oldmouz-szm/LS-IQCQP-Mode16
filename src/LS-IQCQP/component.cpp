...
namespace solver
{
    ...
    void qp_solver::update_best_solution()
    {
        ...
        {
            // Validate only improving incumbents. Validation stays out of the
            // hot path and never mutates the current search state.
            Float max_violation = 0;
            if (strict_incumbent_validation_enabled
                && !validate_assignment(_cur_assignment, &max_violation))
            {
                feasibility_recheck_rejected_total++;
                if (feasibility_recheck_rejected_total <= 5)
                {
                    std::cerr << "[FEASIBILITY_RECHECK] rejected improving incumbent at step "
                              << _steps << ", max_constraint_violation = "
                              << std::setprecision(15) << max_violation
                              << ", assignment_size = " << _cur_assignment.size()
                              << ", variable_size = " << _vars.size() << std::endl;
                    for (size_t i = 0; i < _vars.size() && i < _cur_assignment.size(); ++i)
                    {
                        if (!is_value_legal_for_var(static_cast<int>(i), _cur_assignment[i]))
                        {
                            std::cerr << "  invalid variable " << _vars[i].name
                                      << ": value=" << _cur_assignment[i]
                                      << ", lower=" << _vars[i].lower
                                      << ", upper=" << _vars[i].upper
                                      << ", has_lower=" << _vars[i].has_lower
                                      << ", has_upper=" << _vars[i].has_upper
                                      << ", is_int=" << _vars[i].is_int
                                      << ", is_bin=" << _vars[i].is_bin << std::endl;
                            break;
                        }
                    }
                }

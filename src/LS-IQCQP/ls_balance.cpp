...
namespace solver
{
    ...
    void qp_solver::local_search_mix_balance()
    {
        ...
        // sta_cons();
        Float last_seen_best_obj = LDBL_MAX;
        ...
        for (_steps = 0; _steps <= _max_steps; _steps++)
        {
            ...
            // Phase H: sample expensive global violation tracking only when the
            // adaptive block mechanism can use it.
            if (block_move_enabled && block_repair_enabled && !is_cur_feasible
                && block_trigger_interval > 0 && _steps % block_trigger_interval == 0) {
                Float cur_viol = total_constraint_violation();
                if (cur_viol < best_total_violation_seen - 1e-6) {
                    best_total_violation_seen = cur_viol;
                    last_violation_improve_step = _steps;
                }
            } else if (is_cur_feasible) {
                if (last_feasible_step == -1) {
                    last_feasible_step = _steps;
                }
                if (block_disable_repair_after_first_feasible && block_repair_enabled) {
                    block_repair_enabled = false;
                    block_repair_disabled_after_first_feasible_total++;
                }
                if (TimeElapsed() < _cut_off * block_easy_instance_disable_ratio && block_repair_only_until_feasible) {
                    if (block_repair_enabled) {
                        block_repair_enabled = false;
                        block_repair_disabled_easy_instance_total++;
                    }
                }
                if (_best_object_value < last_seen_best_obj - 1e-6) {
                    last_seen_best_obj = _best_object_value;
                    last_best_update_step = _steps;
                }
            }
            ...
            if (is_cur_feasible)
            {
                ...
                else 
                {
                    ...
                    if (block_move_enabled && block_improve_enabled && should_try_block_improve()) {
                        auto t0 = std::chrono::steady_clock::now();
                        insert_block_operations_mix(false);
                        block_move best_mv;
                        Float block_score = INT32_MIN;
                        select_best_operation_block_mix(best_mv, block_score);
                        auto t1 = std::chrono::steady_clock::now();
                        block_time_spent += std::chrono::duration<double>(t1 - t0).count();
                        
                        if (block_score > 0) {
                            block_selected_total++;
                            if (execute_block_move(best_mv)) {
                                block_executed = true;
                            } else {
                                print_block_failure_sample(best_mv, diagnose_block_move(best_mv), block_score, score, score_equal, "feasible_branch_selected");
                            }
                        }
                    }
            ...
            else 
            {
                ...
                {
                    bool block_executed = false;
                    if (block_move_enabled && block_repair_enabled && should_try_block_repair()) {
                        auto t0 = std::chrono::steady_clock::now();
                        insert_block_operations_mix(true);
                        block_move best_mv;
                        Float block_score = INT32_MIN;
                        select_best_operation_block_mix(best_mv, block_score);
                        auto t1 = std::chrono::steady_clock::now();
                        ...
                        if (block_score > 0) {
                            ...
                            if (execute_block_move(best_mv)) {
                                ...
                                print_block_failure_sample(best_mv, diagnose_block_move(best_mv), block_score, score, INT32_MIN, "infeasible_branch_selected");
                            }
                        }
                    }
        ...
        Float best_max_violation = 0;
        bool best_solution_valid = _best_object_value != INT32_MAX
            && validate_assignment(_best_assignment, &best_max_violation);
        if (!best_solution_valid)
        {
            cout << INT64_MAX << endl;
            cout << "solution_status = NO_FEASIBLE_SOLUTION" << endl;
            cout << "max_constraint_violation = N/A" << endl;
            if (block_move_enabled) print_block_statistics();
        }
        else
        {
            if (is_minimize)
            ...
            cout << endl;
            cout << "solution_status = FEASIBLE" << endl;
            cout << "max_constraint_violation = " << std::setprecision(15) << best_max_violation << endl;
            print_best_solution();
            if (block_move_enabled) print_block_statistics();
